...
import (
	...
	"github.com/starwu/sql2pb/tools/stringx"
...
func (m Message) GenRpcUpdateReqMessage(buf *bytes.Buffer) {
	...
	var filedTag int
	for _, field := range m.Fields {
		if isInSlice([]string{"create_time", "update_time", "version"}, field.Name) {
			continue
		}
		filedTag++
		field.tag = filedTag
...
func (m Message) GenRpcSearchReqMessage(buf *bytes.Buffer) {
	...
	curFields := []MessageField{
		{Typ: "int64", Name: "offset", tag: 1, Comment: "offset"},
		{Typ: "int64", Name: "limit", tag: 2, Comment: "limit"},
		{Typ: "string", Name: "sortKey", tag: 3, Comment: "sortKey"},
		{Typ: "string", Name: "sortType", tag: 4, Comment: "limit"},
...
func (m Message) GenRpcGetListAllReqMessage(buf *bytes.Buffer) {
	...
	m.Fields = []MessageField{
		{Typ: "int64", Name: "delState", tag: 1, Comment: "delState"},
	}
	buf.WriteString(fmt.Sprintf("%s\n", m))
